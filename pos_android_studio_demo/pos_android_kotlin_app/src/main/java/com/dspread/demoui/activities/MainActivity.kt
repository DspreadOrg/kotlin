...
//    private EditText statusEditText, blockAdd, status,status11
    ...
    override fun onGuideListener(btn: Button?) {
        when(btn?.id){
            R.id.doTradeButton -> showGuideView.show(btnDisconnect,this@MainActivity,getString(R.string.msg_disconnect))
            R.id.disconnect -> showGuideView.show(btnUSB,this@MainActivity,getString(R.string.msg_conn_usb))
            R.id.btnBT -> showGuideView.show(doTradeButton,this@MainActivity,getString(R.string.msg_do_trade))
        }
    }
    ...
    private fun generateAdapterData() : List<Map<String, Any>>{
        ...
        for ( dev:BluetoothDevice in lstDevScanned) {
            ...
            itm.put("ICON", if(dev.bondState == BluetoothDevice.BOND_BONDED) Integer.valueOf(R.drawable.bluetooth_blue) else Integer.valueOf(R.drawable.bluetooth_blue_unbond))
    ...
    private fun refreshAdapter() {
        ...
        m_Adapter = MyListViewAdapter(this@MainActivity, data as MutableList<Map<String, Any>>,mLayoutInflater)
        m_ListView!!.adapter = (m_Adapter)
        TRACE.w("m_Adapter$m_Adapter")
        setListViewHeightBasedOnChildren(m_ListView!!)
    }

    private class MyListViewAdapter(private val context : Context, private val m_DataMap :MutableList<Map<String, Any>>,private val m_Inflater:LayoutInflater): BaseAdapter() {
        fun clearData() {
            m_DataMap.clear()
        }
...
//        status11 = (EditText) findViewById(R.id.status11)
        ...
        var cmdAdapter =  ArrayAdapter<String>(this@MainActivity, android.R.layout.simple_spinner_item, cmdList)
        ...
        var spinneradapter =  ArrayAdapter<String>(this@MainActivity, android.R.layout.simple_spinner_item, keyClass)
...
//        MyPosListener listener = new MyPosListener()
        ...
        var sdkVersion = QPOSService.getSdkVersion()
...
//            0~Integer.MAX_VALUE
        ...
        //update ipek
            ...
            var appTLV = ""
...
//					content += getString(R.string.ksn) + " " + ksn + "\n"
                    ...
                    if (!TextUtils.isEmpty(trackksn) && !TextUtils.isEmpty(encTrack2)) {
                        val clearPan = DUKPK2009_CBC.getDate(trackksn, encTrack2, DUKPK2009_CBC.Enum_key.DATA, DUKPK2009_CBC.Enum_mode.CBC)
                    ...
                    if (!TextUtils.isEmpty(pinKsn) && !TextUtils.isEmpty(pinBlock) && !TextUtils.isEmpty(realPan)) {
                        val date = DUKPK2009_CBC.getDate(pinKsn, pinBlock, DUKPK2009_CBC.Enum_key.PIN, DUKPK2009_CBC.Enum_mode.CBC)
...
//				TRACE.w("swipe card:" + content)
        ...
        override fun onQposInfoResult(posInfoData:Hashtable<String, String>) {
            ...
            val isSupportedTrack1 = if (posInfoData.get("isSupportedTrack1") == null)  "" else posInfoData.get("isSupportedTrack1")
            val isSupportedTrack2 = if(posInfoData.get("isSupportedTrack2") == null) "" else posInfoData.get("isSupportedTrack2")
            val isSupportedTrack3 = if(posInfoData.get("isSupportedTrack3") == null) "" else posInfoData.get("isSupportedTrack3")
            val bootloaderVersion = if(posInfoData.get("bootloaderVersion") == null) "" else posInfoData.get("bootloaderVersion")
            val firmwareVersion = if(posInfoData.get("firmwareVersion") == null) "" else posInfoData.get("firmwareVersion")
            val isUsbConnected = if(posInfoData.get("isUsbConnected") == null) "" else posInfoData.get("isUsbConnected")
            val isCharging = if(posInfoData.get("isCharging") == null) "" else posInfoData.get("isCharging")
            val batteryLevel = if(posInfoData.get("batteryLevel") == null) "" else posInfoData.get("batteryLevel")
            val batteryPercentage = if(posInfoData.get("batteryPercentage") == null) "" else posInfoData.get("batteryPercentage")
            val hardwareVersion = if(posInfoData.get("hardwareVersion") == null) "" else posInfoData.get("hardwareVersion")
            val SUB = if(posInfoData.get("SUB") == null) "" else posInfoData.get("SUB")
            val pciFirmwareVersion = if(posInfoData.get("PCI_firmwareVersion") == null) "" else posInfoData.get("PCI_firmwareVersion")
            val pciHardwareVersion = if(posInfoData.get("PCI_hardwareVersion") == null) "" else posInfoData.get("PCI_hardwareVersion")
            var content = ""
...
//			}
        ...
        override fun onRequestTransactionResult(transactionResult:TransactionResult) {
            ...
            if (transactionResult == TransactionResult.APPROVED) {
                ...
                var message = getString(R.string.transaction_approved) + "\n" + getString(R.string.amount) + ": $" + amount + "\n"
                if (cashbackAmount != "") {
...
//                    deviceShowDisplay("DECLINED")
            ...
            dialog!!.findViewById<Button>(R.id.confirmButton).setOnClickListener{dismissDialog()}
            dialog!!.show()
            amount = ""
            cashbackAmount = ""
        }
        ...
        override fun onRequestSelectEmvApp(appList:ArrayList<String>) {
            ...
            appListView.setAdapter(ArrayAdapter<String>(this@MainActivity, android.R.layout.simple_list_item_1, appNameList))
        ...
        override fun onRequestSetAmount() {
            ...
            var transactionTypes = arrayOf("GOODS", "SERVICES", "CASH", "CASHBACK", "INQUIRY",
            ...
            ( dialog!!.findViewById(R.id.transactionTypeSpinner) as (Spinner)).setAdapter( ArrayAdapter<String>(this@MainActivity, android.R.layout.simple_spinner_item,
                    transactionTypes))

            dialog!!.findViewById<Button>(R.id.setButton).setOnClickListener(object :View.OnClickListener {
                override fun onClick(v: View?) {

                    var amount = ((dialog!!.findViewById<EditText>(R.id.amountEditText))).getText().toString()
                    var cashbackAmount = ( (dialog!!.findViewById<EditText>(R.id.cashbackAmountEditText))).getText().toString()
                    var transactionTypeString = ( dialog!!.findViewById(R.id.transactionTypeSpinner) as (Spinner)).getSelectedItem()
...
//                    analyData(tlv)// analy tlv ,get the tag you need
    ...
    //									str = ""
        ...
        override fun onRequestTime() {
            ...
            dismissDialog()
            var terminalTime =  SimpleDateFormat("yyyyMMddHHmmss").format(Calendar.getInstance().getTime())
        ...
        override fun onRequestQposConnected() {
            ...
            Toast.makeText(this@MainActivity, "onRequestQposConnected", Toast.LENGTH_LONG).show()
            dismissDialog()
            var use_time = Date().time - start_time
            // statusEditText.setText(getString(R.string.device_plugged))
            statusEditText.setText(getString(R.string.device_plugged) + "--" + getResources().getString(R.string.used) + QPOSUtil.formatLongToTimeStr(use_time, this@MainActivity))
            doTradeButton.isEnabled = true
            btnDisconnect.isEnabled = true
            btnQuickEMV.isEnabled = true
            setTitle(mTitle +"("+blueTitle.substring(0,6)+"..."+blueTitle.substring(blueTitle.length-3,blueTitle.length)+")")
        }
        ...
        override fun onDeviceFound(arg0:BluetoothDevice) {
            if (arg0 != null) {
                ...
                if(m_Adapter != null){
                    ...
                    itm["ICON"] = if(arg0.bondState == BluetoothDevice.BOND_BONDED) Integer.valueOf(R.drawable.bluetooth_blue) else Integer.valueOf(R.drawable.bluetooth_blue_unbond)
...
//				statusEditText.setText("onReadWriteMifareCardResult fail"+msg)
    ...
    private var terminalTime =  SimpleDateFormat("yyyyMMddHHmmss").format(Calendar.getInstance().getTime())
    ...
    inner class MyOnClickListener : OnClickListener{
        override fun onClick(v: View?) {
            ...
            if (selectBTFlag) {
                ...
                terminalTime = SimpleDateFormat("yyyyMMddHHmmss").format(Calendar.getInstance().time)
...
//                    allFiles = FileUtils.getAllFiles(FileUtils.POS_Storage_Dir)
                        ...
                        for (fileName:String in allFiles) {
                            if (!TextUtils.isEmpty(fileName)) {
                                if (fileName.toUpperCase().endsWith(".asc".toUpperCase())) {
                                    ...
                                    Toast.makeText(this@MainActivity, "Upgrade package path:" +
                    ...
                    if (a == -1) {
                        Toast.makeText(this@MainActivity, "please keep the device charging", Toast.LENGTH_LONG).show()
                        return
                    }
...
//      update emv config by bin files
        val emvAppCfg = QPOSUtil.byteArray2Hex(FileUtils.readAssetsLine("emv_app.bin", this@MainActivity))
        val emvCapkCfg = QPOSUtil.byteArray2Hex(FileUtils.readAssetsLine("emv_capk.bin", this@MainActivity))
